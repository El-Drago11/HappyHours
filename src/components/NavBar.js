    ...
    <nav className="navbar navbar-expand-lg sticky-top"style={{lineHeight:'3rem'}}>
      <div className="container-fluid p-0" >
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          {!showLang ? <div button type="button" className="btn btn-info mx-md-3 p-2" onClick={()=>handleGpt()}><b>GPT</b>{<Search/>}</div> : <div button type="button" className="btn btn-danger mx-1 mx-md-3 p-2" onClick={()=>handleHome()}><b>Home</b></div>}
          <span className='text-light mx-1 mx-md-3'><b>{userData[0]?.email.slice(0,7)}..</b></span>
          <div button type="button" className="btn btn-danger mx-1 me-2" onClick={()=>logout()}>LogOut</div>
          <Link to="/Home/WatchLater" button type="button" className=" mx-md-3 text-light bg-dark my-2" style={{border:'none',objectFit:'contain',mixBlendMode:'color-burn'}}><span className='position-relative p-2'><WatchLaterIcon/><span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">{cartItem.length}<span className="visually-hidden">WatchLater</span></span></span></Link>
        </div>
      </div>
    </nav>
  </div>
