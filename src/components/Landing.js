      ...
      <div className='container' style={{width:'25rem'}}>
        ...
        <form className='container mx-auto text-light bg-black bg-opacity-75 p-5' onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="my-3  p-2">
            <p className='text-light'>{isSign ? "New to Happy-Hours ?  " : "Already a Registered ?  "} <span className='text-danger font-weight-bold' role='button' onClick={() => formSign()}><b>{isSign ? "  Sign up Now" : " Login Now "}</b></span></p>
          </div>
        </form>
      </div>
    </div>
